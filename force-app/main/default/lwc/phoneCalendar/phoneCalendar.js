...
import labelNoPermission from "@salesforce/label/c.PhoneCal_NoPermission";
import hasUsePhoneCalendar from "@salesforce/customPermission/UsePhoneCalendar";
...
export default class PhoneCalendar extends NavigationMixin(LightningElement) {
  // eslint-disable-line
  // ── Core state ──────────────────────────────────────────────────────────────
  @track anchorDateStr = "";
  ...
  label = {
    ...
    noPermission: labelNoPermission
  };
  ...
  get anchorYear() {
    return parseInt(this.anchorDateStr.split("-")[0], RADIX_DECIMAL);
  }
  get anchorMonth() {
    return parseInt(this.anchorDateStr.split("-")[1], RADIX_DECIMAL) - 1;
  }
  get anchorDayNumber() {
    return parseInt(this.anchorDateStr.split("-")[2], RADIX_DECIMAL);
  }

  // ── Permission check ─────────────────────────────────────────────────────────

  get hasPermission() {
    return hasUsePhoneCalendar;
  }
  ...
  get popupDayClass() {
    return `action-opt${this.isDayView ? " action-opt_active" : ""}`;
  }
  get popup3DayClass() {
    return `action-opt${this.is3DayView ? " action-opt_active" : ""}`;
  }
  get popupMonthClass() {
    return `action-opt${this.isMonthView ? " action-opt_active" : ""}`;
  }
  get popupAgendaClass() {
    return `action-opt${this.isAgendaView ? " action-opt_active" : ""}`;
  }

  // ── Period label ────────────────────────────────────────────────────────────

  get periodLabel() {
    if (!this.anchorDateStr) {
      return "";
    }
    ...
    if (this.viewMode === "agenda") {
      return (
        new Intl.DateTimeFormat(this.locale, {
          ...
          month: "long",
          day: "numeric"
        }).format(anchor) + this.label.agendaSuffix
      );
    }
    return "";
  }
  ...
  get showCurrentTimeInCol0() {
    return (
      this.is3DayView && this.threeDayDates[0] === this.toDateStr(new Date())
    );
  }

  get showCurrentTimeInCol1() {
    return (
      this.is3DayView && this.threeDayDates[1] === this.toDateStr(new Date())
    );
  }

  get showCurrentTimeInCol2() {
    return (
      this.is3DayView && this.threeDayDates[2] === this.toDateStr(new Date())
    );
  }
  ...
  get dayViewAllDayEvents() {
    return this.events.filter(
      (e) => e.IsAllDayEvent && e.ActivityDate === this.anchorDateStr
    );
  }
  ...
  get dayViewEvents() {
    const filtered = this.events.filter(
      (e) =>
        !e.IsAllDayEvent &&
        e.ActivityDateTime &&
        this.eventOverlapsDate(e, this.anchorDateStr)
    );
  ...
  get threeDayDateHeaders() {
    ...
    return this.threeDayDates.map((dateStr, idx) => {
      ...
      return {
        id: `tdh-${idx}`,
        dow: new Intl.DateTimeFormat(this.locale, { weekday: "narrow" }).format(
          d
        ),
  ...
  get threeDayEventsCol0() {
    return this.getEventsForDate(this.threeDayDates[0]);
  }
  get threeDayEventsCol1() {
    return this.getEventsForDate(this.threeDayDates[1]);
  }
  get threeDayEventsCol2() {
    return this.getEventsForDate(this.threeDayDates[2]);
  }
  ...
  getEventsForDate(dateStr) {
    ...
    const filtered = this.events.filter(
      (e) =>
        !e.IsAllDayEvent &&
        e.ActivityDateTime &&
        this.eventOverlapsDate(e, dateStr)
    );
  ...
  eventOverlapsDate(event, dateStr) {
    ...
    const endDateStr = this.toDateStr(
      new Date(new Date(event.EndDateTime).getTime() - 1)
    );
  ...
  layoutEvents(rawEvents, dateStr) {
    ...
    const items = rawEvents.map((e) => {
      ...
      const rawEndDt = e.EndDateTime
        ...
        : new Date(
            startDt.getTime() + DEFAULT_DURATION_MINUTES * MS_PER_MINUTE
          );
      ...
      return {
        e,
        startMin,
        endMin: startMin + clippedDuration,
        totalDurationMin
      };
    });
      ...
      if (dayEvents.length > 0) {
        ...
        groups.push({
          ...
          events: dayEvents.map((e) => {
            ...
            const color =
              USER_COLORS[Math.max(colorIdx, 0) % USER_COLORS.length];
            return {
              ...e,
              colorBarStyle: `background-color: ${color.border};`
            };
          })
        });
      }
  ...
  get atMaxUsers() {
    return this.selectedUserIds.length >= MAX_SELECTED_USERS;
  }
  ...
  get filteredCalendarsWithState() {
    ...
    return this.availableCalendars
      ...
      .map((c, idx) => {
        ...
        const colorIdx =
          selectedIdx >= 0 ? selectedIdx : this.availableUsers.length + idx;
        return {
          ...c,
          isSelected,
          isDisabled,
  ...
  get filteredUsersWithState() {
    const term = (this.userSearchTerm || "").trim().toLowerCase();
    if (!term) {
      return this.availableUsersWithState;
    }
    return this.availableUsersWithState.filter((u) =>
  ...
  renderedCallback() {
    if (!this.touchMoveListenerAdded) {
      const el = this.template.querySelector(".calendar-root");
      if (el) {
        el.addEventListener("touchmove", this.touchMoveHandler, {
          passive: false
        });
  ...
  loadEvents() {
    ...
    getEventsForMonth({
      ...
      .catch((error) => {
        this.showToast(
          this.label.error,
          error.body?.message || this.label.errorLoad,
          "error"
        );
      })
      .finally(() => {
  ...
  getDateRangeForView() {
    ...
    if (this.viewMode === "month") {
      return {
        start: this.toDateStr(new Date(y, m, 1)),
        end: this.toDateStr(new Date(y, m + 1, 0))
      };
    }
    if (this.viewMode === "day") {
  ...
  buildCalendar() {
    ...
    for (let i = 0; i < firstDow; i++) {
      days.push({
        id: `pad-${i}`,
        dayNumber: "",
        cellClass: "slds-col day-cell day-cell_empty",
        labelClass: "day-number",
        dateStr: "",
        eventDots: []
      });
    }
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const eventDots = ownerIds.slice(0, MAX_EVENT_DOTS).map((oid, idx) => {
        ...
        return {
          id: `dot-${dateStr}-${idx}`,
          dotStyle: `background-color: ${color.border};`
        };
      });
      ...
      if (isSun) {
        cellClass += " day-cell_sun";
      }
      if (isSat) {
        cellClass += " day-cell_sat";
      }
  ...
  handleTodayClick() {
    const todayStr = this.toDateStr(new Date());
    if (this.anchorDateStr === todayStr && this.viewMode !== "month") {
      return;
    }
  ...
  handleUserToggle(event) {
    ...
    if (idx >= 0) {
      ...
      if (this.selectedUserIds.length >= MAX_SELECTED_USERS) {
        return;
      }
  ...
  savePreference() {
    saveCalendarPreference({
      selectedUserIds: this.selectedUserIds.join(",")
    }).catch(() => {});
  }
  ...
  get hasSelectedDayEvents() {
    return this.selectedDayEvents.length > 0;
  }
  ...
  formatAgendaDateLabel(date, isToday) {
    ...
    if (isToday) {
      return `${this.label.today} ${formatted}`;
    }
    return formatted;
  }
